...
export async function GET(req: NextRequest) {
  ...
  // Load image map if available (generated by scripts/upload-wp-images.mjs)
  let imageMap: Record<string, string> = {}
  try {
    const mapPath = path.join(process.cwd(), 'scripts', 'image-map.json')
    imageMap = JSON.parse(readFileSync(mapPath, 'utf-8'))
  } catch { /* no image map — images will be skipped */ }
  ...
  function buildData(post: WPPost) {
    const coverImageId = post.coverImageUrl ? imageMap[post.coverImageUrl] : undefined
    return {
      title: post.title,
      excerpt: post.excerpt || undefined,
      body: htmlToLexical(post.content, imageMap),
